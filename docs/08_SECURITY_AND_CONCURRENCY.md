...
```txt
用户只能查看自己所在会话
删除好友后，双方都不能再查看、搜索或发送到旧 private conversation
退出群聊后，当前用户不能再查看、搜索或发送该群消息
重新入群后，当前用户只能查看本次 joined_at 之后产生的群消息
用户只能删除自己视角的消息
...
建议计算 sha256
不提供公开静态目录直接下载
MVP 使用本地 uploads 目录，通过 FILE_LOCAL_PATH 配置
只有上传者或拥有可见文件消息的会话成员可以下载
文件消息可见性必须同时遵守删除好友、退出群聊、重新入群 joined_at、单条删除、清空和撤回过滤规则
```
